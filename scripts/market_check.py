...
def get_category_pe_ratios() -> dict:
    """Returns PE ratios for Large, Mid, and Small Cap."""
...
def get_pe_status(cap_type: str, pe: float) -> str:
    """Helper to get PE Valuation status text."""
    if cap_type == "large":
...
def get_midcap150_symbols() -> list:
    return ["FEDERALBNK.NS", "VOLTAS.NS", "POLYCAB.NS"]

def get_smallcap250_symbols() -> list:
    return ["CDSL.NS", "ANGELONE.NS", "KEI.NS"]
...
def get_category_stage(drawdown_52w: float, weekly_rsi: float, pe_ratio: float, vix: float) -> dict:
    if drawdown_52w < 1.5 or weekly_rsi >= 72.0:
        return {
            ...
            "lumpsum_pct": "Pause SIP & Lumpsum (Prepay Loan) 🔴",
            "short_action": "Pause SIP (Prepay Loan) 🏦"
        }
...
• PE Ratio: {pe_data['large_pe']:.2f} ({large_pe_status})
...
• PE Ratio: {pe_data['mid_pe']:.2f} ({mid_pe_status})
...
• PE Ratio: {pe_data['small_pe']:.2f} ({small_pe_status})
...
1. 🔥 Extreme High (All-Time Peak)
   └ 🔴 Stop SIP | Prepay Loan / Save Cash
